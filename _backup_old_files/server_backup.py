...
try:
    ...
    print("Warning: PyMuPDF not available, using alternative PDF processing")
...
def normalize_medicine_name(name):
    """Enhanced normalize medicine names for better matching - keeps brand name intact"""
    if not name:
        return ""
    ...
    # Remove SPF values, percentages, and dosage information FIRST (before removing product types)
    name = re.sub(r'\s*spf\s*[-\s]*\d+\s*%?\s*', ' ', name)  # Remove SPF-40%, SPF 40, etc.
    name = re.sub(r'\s*\d+\s*%\s*', ' ', name)  # Remove 1%, 2%, etc.
    name = re.sub(r'\s*\d+\s*mg\s*', ' ', name)
    name = re.sub(r'\s*\d+\s*gm\s*', ' ', name)
    name = re.sub(r'\s*\d+\s*ml\s*', ' ', name)
    name = re.sub(r'\s*\d+\s*mcg\s*', ' ', name)
    
    # Remove product type words ONLY if they're at the start or end (preserve brand names)
    product_types = ['tab', 'tablet', 'cap', 'capsule', 'syrup', 'syp', 'inj', 'injection', 
                     'susp', 'suspension', 'drops', 'sol', 'solution', 'cream', 'ointment', 
                     'lotion', 'gel', 'balm', 'oral']
    
    words = name.split()
    if len(words) > 1:
        # Remove product type only if it's the first or last word
        if words[0] in product_types:
            words = words[1:]
        if len(words) > 0 and words[-1] in product_types:
            words = words[:-1]
        name = ' '.join(words)
    ...
    # Common OCR errors - but be specific
    # Only replace if it's a complete word, not part of a word
    words = name.split()
    words = ['hmf' if w == 'nmfe' else w for w in words]
    name = ' '.join(words)
...
def extract_brand_name(name):
    """Extract the core brand/medicine name (first significant word)"""
    if not name:
        return ""
    ...
    # Get the first significant word (at least 3 characters)
    for word in words:
        if len(word) >= 3:
            return word
...
def fuzzy_match_score(str1, str2):
    ...
    # Check for partial word matches with lower threshold
    for word1 in words1:
        for word2 in words2:
            if len(word1) >= 3 and len(word2) >= 3:  # Lowered from 4 to 3
                word_ratio = SequenceMatcher(None, word1, word2).ratio()
                if word_ratio > 0.7:  # Lowered from 0.8 to 0.7
                    partial_word_score = word_ratio * 0.85
                    base_score = max(base_score, partial_word_score)
    
    # Check if any significant word from one appears in the other (even partially)
    for word1 in words1:
        if len(word1) >= 4:
            for word2 in words2:
                if len(word2) >= 4:
                    ...
                    if word1.startswith(word2[:3]) or word2.startswith(word1[:3]):
                        prefix_score = 0.75
                        base_score = max(base_score, prefix_score)
...
def find_best_match(medicine_name, bill_items, threshold=0.5):
...
def extract_images_from_pdf(pdf_bytes: bytes) -> list:
    """
    Extract all images from a PDF file using available libraries
    Returns a list of dicts: {bytes, mime, page, source}
    """
    images = []
    ...
    if PYMUPDF_AVAILABLE:
        try:
            ...
            pdf_document.close()
            print(f"✓ PyMuPDF extracted {len(images)} pages from PDF")
            return images
            
        except Exception as e:
            print(f"✗ PyMuPDF error: {e}. Trying alternative method...")
            images = []
    
    # Fallback to pdf2image (requires poppler)
    if PDF2IMAGE_AVAILABLE:
        try:
            print("Attempting PDF extraction with pdf2image...")
            ...
            print(f"✓ pdf2image extracted {len(images)} pages from PDF")
            return images
    ...
    # Last resort - use PyPDF2 to at least count pages
    if PYPDF2_AVAILABLE:
        try:
            pdf_reader = PyPDF2.PdfReader(io.BytesIO(pdf_bytes))
            num_pages = len(pdf_reader.pages)
            print(f"✗ PDF has {num_pages} pages, but no image extraction library is working.")
            print("  SOLUTION: Install Poppler for Windows")
    ...
    print("")
    print("="*60)
    print("ERROR: Cannot process PDF files")
    print("="*60)
    print("No working PDF processing library found.")
    print("")
    print("QUICK FIX:")
    ...
    print("")
    print("OR: Convert your PDF pages to JPG/PNG images and upload those.")
    print("="*60)
    return []

def process_file_or_pdf(file_obj) -> list:
    """
    Process a file - if it's a PDF, extract images; if it's an image, return as-is
    Returns a list of dicts with: bytes, mime, filename, page (optional)
    """
    ...
    if mime_type == 'application/pdf' or filename.lower().endswith('.pdf'):
        ...
        if not images:
            # PDF extraction failed - return error result
            results.append({
                'bytes': None,
                ...
                'error_message': 'Poppler not installed. Please install Poppler or convert PDF to images.'
            })
        else:
...
def ocr_auto():
    ...
    # Process each uploaded file (could be image or PDF)
    for f in files:
        # Process file - if PDF, extract all pages as images
        processed_files = process_file_or_pdf(f)
        
        # Analyze each image (from PDF pages or direct image upload)
        for proc_file in processed_files:
            filename = proc_file['filename']
            file_bytes = proc_file.get('bytes')
            mime_type = proc_file['mime']
            is_from_pdf = proc_file['is_pdf']
            page_num = proc_file.get('page')
            original_filename = proc_file['original_filename']
            ...
            if 'error' in proc_file:
                results.append({
                    ...
                    "errorMessage": proc_file['error_message'],
                    "prescriptionNames": [],
                    "testNames": [],
                    "billItems": []
                })
            ...
            # Skip if no bytes (shouldn't happen but just in case)
            if not file_bytes:
                continue
            ...
            if not api_key:
                ...
                try:
                    data = fetch_with_cache('enhanced_auto', file_bytes, mime_type, enhanced_prompt, api_key)
                except requests.RequestException as e:
                    results.append({
                        "filename": filename,
                        "originalFilename": original_filename,
            ...
            if isinstance(data, dict):
                ...
                file_result = {
                    ...
                    "isPDF": is_from_pdf,
                    "page": page_num if is_from_pdf else None
                }
